...
//!
//! * The workspace root declares **exactly seven** member crates (Requirement 1.1).
...
#[test]
fn workspace_declares_exactly_seven_expected_members() {
    ...
    assert_eq!(
        members.len(),
        7,
        "workspace must declare exactly seven member crates, found {}: {:?}",
        members.len(),
        members
    );
