...
//! [`ClusterMetadata`](vela_core::ClusterMetadata), the node's running partition
//! drivers must be brought into line with the new catalogue: a driver is started
//! for every partition this node now replicates and stopped for every partition
//! that is gone or no longer assigned to it. That alignment is
//! **reconciliation**.
//!
...
//!
//! The diff itself is the pure, deterministic [`plan_reconcile`] lifted into
//! [`vela_core::reconcile`] so the server and the deterministic simulation
//! harness share one planner and cannot diverge. This module owns only the
//! `tokio` task wrapper and the spawn/stop side-effects.

use std::collections::HashSet;
use std::sync::Arc;

use tokio::time::{interval, MissedTickBehavior};
use vela_core::plan_reconcile;
...
/// opposite order), computes the diff via [`plan_reconcile`], then applies it by
/// reusing `spawn_partition` / `stop_partition`.
///
...
mod tests {
    ...
    use vela_core::{LogBackend, NodeId, Partition, PartitionIndex, Topic, TopicState};
    ...
    // --- Spawn-failure skip path (Requirement 6.7) ---------------------------
    //
    // The pure planner diff ([`plan_reconcile`]) is owned and tested in
    // [`vela_core::reconcile`]. The tests below drive the full [`reconcile`]
    // pass against a real [`NodeShared`] to prove the side-effecting wrapper's
